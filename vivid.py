...
# Tracks Telegram message IDs for UI operations (clean, etc.).
# Decoupled from chat_history which is strictly for the LLM context.
ui_message_tracker = []
chat_history = []
...
def save_to_json(name):
    """
    Saves the current session to two separate files in CHATS_DIR:
      [name].json      - contains only the chat_history array (pure LLM context).
      [name]_meta.json - contains session metadata (model, role, scenes, etc.).
    """
    base_name = f"{VIVID_PREFIX}_{name}" if name == "last_session" else name
    history_path = CHATS_DIR / f"{base_name}.json"
    meta_path = CHATS_DIR / f"{base_name}_meta.json"
    ...
    # Write history (pure LLM messages only)
...
def load_from_json(name):
    ...
    Supports the new split-file format ([name].json + [name]_meta.json) and
    the legacy single-file format ({ "history": [...], "metadata": {...} }).
    ...
    # Resolve base file name (support legacy prefix)
    base_name = clean_name
    history_path = (CHATS_DIR / f"{base_name}.json").resolve()
    if not history_path.exists():
        base_name = f"{VIVID_PREFIX}_{clean_name}"
        history_path = (CHATS_DIR / f"{base_name}.json").resolve()
    ...
    try:
        ...
        # --- New split-file format: history file contains a plain list ---
        if isinstance(data, list):
            chat_history = data
            # Load metadata from companion _meta.json if it exists
            meta_path = (CHATS_DIR / f"{base_name}_meta.json").resolve()
            if meta_path.exists():
        ...
        # --- Legacy single-file format: { "history": [...], "metadata": {...} } ---
        if isinstance(data, dict) and "history" in data:
            chat_history = data["history"]
            # Strip any legacy UI keys from history entries so the LLM context stays pure
            for msg in chat_history:
                msg.pop("msg_ids", None)
                msg.pop("is_command", None)
                msg.pop("incoming", None)
        ...
        # Fallback: plain list without wrapper (very old format)
        chat_history = data
        return True
...
async def reply_and_log(update, text, *args, **kwargs):
    ...
    async def send_chunk(content):
        try:
            return await update.message.reply_text(content, parse_mode='Markdown')
        except Exception:
            # Fallback to plain text if Markdown parsing fails (e.g. due to unescaped special characters)
            return await update.message.reply_text(content)

    for para in paragraphs:
        # Check if adding this paragraph exceeds the limit
        if len(current_message) + len(para) + 2 > 4000:
            # Send current chunk and track its ID
            sent_msg = await send_chunk(current_message)
            ui_message_tracker.append(sent_msg.message_id)
            current_message = para + "\n\n"
        else:
            current_message += para + "\n\n"
            
    # Send the final chunk and track its ID
    if current_message:
        sent_msg = await send_chunk(current_message.strip())
        ui_message_tracker.append(sent_msg.message_id)
...
async def reply_transient(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str, delay: int = 5):
    ...
    try:
        # Track the trigger command message ID before deleting it
        user_msg_id = update.message.message_id
        if user_msg_id not in ui_message_tracker:
            ui_message_tracker.append(user_msg_id)
        ...
        print(f"WARNING: Failed to send transient reply: {e}")
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        "**🎦 Scenes**\n"
        "• `.scene` - List scenes\n"
        "• `.scene 1 -2 3` - Multi-toggle scenes (prefix `-` to remove)\n"
        ...
        "• `.mnsfw [n/next/prev]` - List or cycle NSFW models from URL\n"
...
async def chat_actions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    global chat_history, ui_message_tracker
    words = update.message.text.lower().split() if update.message.text else []
    ...
    if is_clear or is_clean:
        if is_clean:
            ...
            # Physically delete messages from Telegram UI using ui_message_tracker
            for mid in reversed(ui_message_tracker):
                try: await context.bot.delete_message(chat_id=chat_id, message_id=mid)
                except: pass
            ui_message_tracker.clear()

        # Reset history to only the system prompt (pure LLM format, no extra keys)
    ...
    if not context.args:
        ...
        # Exclude _meta.json files and last_session entries from the listing
        chat_names = [
            f.stem for f in files
            if "last_session" not in f.name and not f.stem.endswith("_meta")
        ]
    ...
    if cmd == "save" and len(context.args) > 1:
        ...
        if load_from_json(context.args[1]):
            ...
            await reply_and_log(update, f"❌ Failed to load `{context.args[1]}`. Check if the file exists.")
...
async def resend_last_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i in range(len(chat_history) - 1, -1, -1):
        if chat_history[i]["role"] == "user":
            ...
            chat_history.pop(i)
            # Also remove the bot's following response if it exists
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE, incoming_text=None):
    ...
    # Track the incoming message ID in the UI tracker
    if not incoming_text and update.message:
        user_msg_id = update.message.message_id
        if user_msg_id not in ui_message_tracker:
            ui_message_tracker.append(user_msg_id)
    ...
    # In story mode, the user's prompt is used as context for generating the next
    # narrative passage but is NOT appended to chat_history — only the assistant's
    # response is stored, keeping the history as a clean sequence of story beats.
    ...
    if CURRENT_MODE == "story":
        # Only narrative assistant responses are included in the payload.
        # The current user prompt is appended at the end to guide the next beat.
        narrative_assistants = [m for m in history_subset if m["role"] == "assistant"]
        payload = (
            [{"role": "system", "content": full_system_prompt}]
            + narrative_assistants
            + [{"role": "user", "content": prompt}]
        )
    else:
        # Chat mode: full alternating history is sent to the LLM.
        payload = [{"role": "system", "content": full_system_prompt}] + history_subset
    ...
    try:
        # Use centralized helper — payload is already clean (role + content only)
        reply = await generate_response(payload)
        ...
        for i in range(0, len(reply), 4000):
            ...
            try:
                sent_msg = await update.message.reply_text(chunk, parse_mode='Markdown')
            except Exception:
                sent_msg = await update.message.reply_text(chunk)
            ui_message_tracker.append(sent_msg.message_id)

        # Append assistant response to history (always, in both modes)
...
async def ask_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global ui_message_tracker
    if update.effective_user.id != TELEGRAM_USER_ID:
        return

    # 1. Track the user's ".ask [question]" message ID so it can be cleaned
    user_msg_id = update.message.message_id
    if user_msg_id not in ui_message_tracker:
        ui_message_tracker.append(user_msg_id)

    if not context.args:
        sent_msg = await update.message.reply_text("❌ Please provide a question. Usage: `.ask [your question]`")
        ui_message_tracker.append(sent_msg.message_id)
        return

    question = " ".join(context.args)

    # Show a temporary message saying "thinking..."
    thinking_msg = await update.message.reply_text("thinking...")

    # 2. Build a neutral payload using the ongoing conversation history (context)
    # Exclude system persona at index 0
    history_subset = chat_history[1:][-CONTEXT_LIMIT:]
    ...
    system_content = "You are a helpful assistant. Use the conversation history provided to answer the user's question accurately. Do not adopt any persona."
    if char_blocks:
        system_content += "\n\nACTIVE CHARACTERS IN SCENE:\n" + "\n\n".join(char_blocks)
    if scene_blocks:
        system_content += "\n\nCURRENT SCENE SETTING:\n" + "\n\n".join(scene_blocks)
    ...
    try:
        # 3. Generate response using your standard centralized runner
        ...
        # 4. Reply to the user and ensure ALL fragments are tracked for UI clearing
        for i in range(0, len(reply), 4000):
            chunk = reply[i:i+4000]
            try:
                sent_msg = await update.message.reply_text(chunk, parse_mode='Markdown')
        ...
        # If it fails, we still want to remove the thinking message
        try:
            await thinking_msg.delete()
        except Exception:
            pass
        sent_msg = await update.message.reply_text(f"Error handling request: {e}")
        ui_message_tracker.append(sent_msg.message_id)
...
async def story_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Manages story files in STORIES_DIR.
    - No args: Lists all existing .txt story files.
    - 'save [name]': Extracts all assistant messages from the current chat history
      and saves them as a plain text story file.
    Usage: .story | .story save [name]
    """
    if update.effective_user.id != TELEGRAM_USER_ID:
        return

    # No arguments: list existing story files instead of showing an error
    if not context.args:
        story_files = sorted(STORIES_DIR.glob("*.txt"))
        if not story_files:
            await reply_and_log(update, "📂 No story files found in the stories directory.")
            return
        text = "📖 **Saved Stories:**\n"
        for i, sf in enumerate(story_files):
            size_kb = round(sf.stat().st_size / 1024, 1)
            text += f"{i+1}. `{sf.stem}` ({size_kb} KB)\n"
        text += "\nUse `.story save [name]` to save the current session as a story."
        await reply_and_log(update, text)
        return

    if context.args[0].lower() != "save" or len(context.args) < 2:
...
async def last_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        try:
            ...
            if n <= 0:
                await reply_and_log(update, "❌ Please specify a positive number.")
                return
        except ValueError:
...
async def scene_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Usage:
      .scene                    - List all scenes with on/off status.
      .scene 1 -2 3             - Multi-toggle: add scene 1 & 3, remove scene 2.
      .scene tavern -forest     - Multi-toggle by name.
      .scene all off            - Deactivate all scenes.
      .scene edit [n]           - Get scene description to edit.
      .scene save [name] [text] - Save scene description.

    Any argument prefixed with '-' removes that scene from ACTIVE_SCENES.
    Any argument without '-' adds that scene to ACTIVE_SCENES.
    """
    if update.effective_user.id != TELEGRAM_USER_ID:
        return
    ...
    if not context.args:
        ...
        for i, name in enumerate(available_scenes):
            ...
            text += f"{i+1}. `{name}` {status}\n"
        text += "\nUse `.scene [1 -2 3]` to multi-toggle, or `.scene all off`"
        await reply_and_log(update, text)
        return
    ...
    # --- Multi-toggle: loop through all args ---
    added = []
    removed = []

    for raw_arg in context.args:
        # Determine if this is a removal (prefixed with '-')
        if raw_arg.startswith("-"):
            token = raw_arg[1:]  # strip the leading '-'
            remove = True
        else:
            token = raw_arg
            remove = False

        # Resolve token to a scene name (by index or by name)
        target = None
        ...
        elif token_lower in available_scenes:
            target = token_lower
        else:
            # Try case-insensitive match
            for s in available_scenes:
                if s.lower() == token_lower:
                    target = s
                    break

        if target is None:
            continue  # Skip unrecognised tokens silently

        if remove:
            if target in ACTIVE_SCENES:
                ACTIVE_SCENES.remove(target)
                removed.append(target)
        else:
            if target not in ACTIVE_SCENES:
                ACTIVE_SCENES.append(target)
                added.append(target)

    save_to_json("last_session")

    # Build a compact status reply
    parts = []
    if added:
    ...
    if not parts:
        parts.append("ℹ️ No changes made (scenes already in requested state or not found).")

    await reply_transient(update, context, "\n".join(parts))
...
async def mnsfw_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Lists and allows selection of good NSFW models fetched from a remote URL."""
    global CURRENT_MODEL
    if update.effective_user.id != TELEGRAM_USER_ID: return
    if "online" not in CURRENT_PROVIDER:
        await reply_and_log(update, "⚠️ The `.mnsfw` command is only applicable to `OLLAMA_ONLINE` providers.")
        return
    ...
    try:
        ...
        await reply_and_log(update, f"❌ Failed to fetch NSFW models list: {e}")
        return

    # Parse and strip empty lines
    nsfw_models = [line.strip() for line in nsfw_data.splitlines() if line.strip()]
    if not nsfw_models:
        await reply_and_log(update, "ℹ️ NSFW models list fetched from URL is empty.")
        return

    display_name = provider_display_info.get(CURRENT_PROVIDER, CURRENT_PROVIDER)

    try:
        # 2. Get models from the current OLLAMA_ONLINE provider
        resp = await ollama_clients[CURRENT_PROVIDER].list()
        all_models = getattr(resp, 'models', [])
        all_models.sort(key=lambda x: x.model)
        
        # Filter models returned by provider to only include those in the NSFW list (case-insensitive)
        nsfw_set = {m.lower() for m in nsfw_models}
        good_nsfw = [m.model for m in all_models if m.model.lower() in nsfw_set]

        if not good_nsfw:
            await reply_and_log(update, f"ℹ️ No NSFW models from the list detected on `{display_name}`.")
            return

        if not context.args:
            text = f"🔞 **NSFW Models (`{display_name}`):**\n"
            for i, name in enumerate(good_nsfw):
                active = " ✅" if name == CURRENT_MODEL else ""
                text += f"{i+1}. `{name}`{active}\n"
            await reply_and_log(update, text)
            return
        ...
        try:
            index = int(context.args[0]) - 1
            if 0 <= index < len(good_nsfw):
                CURRENT_MODEL = good_nsfw[index]
                await reply_transient(update, context, f"🧠 Switched to NSFW model: `{CURRENT_MODEL}`")
                save_to_json("last_session")
            else:
                await reply_and_log(update, "❌ Invalid model number.")
        except ValueError:
            ...
            if arg in ["next", "n", "pref", "prev", "p"]:
                try:
                    curr_idx = good_nsfw.index(CURRENT_MODEL)
                except ValueError:
                    curr_idx = -1

                if arg in ["next", "n"]:
                    new_idx = (curr_idx + 1) % len(good_nsfw)
                else:
                    new_idx = (curr_idx - 1) % len(good_nsfw)

                CURRENT_MODEL = good_nsfw[new_idx]
                await reply_transient(update, context, f"🧠 Switched to NSFW model: `{CURRENT_MODEL}`")
                save_to_json("last_session")
            else:
                await reply_and_log(update, "❌ Usage: `.mnsfw [number/next/prev]`")
    except Exception as e:
        await reply_and_log(update, f"❌ Failed to list models: {e}")
...
def main():
    ...
    app.add_handler(PrefixHandler(".","mnsfw", mnsfw_command))
